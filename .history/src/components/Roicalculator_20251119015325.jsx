...
const ROICalculator = () => {
  ...
  useEffect(() => {
    if (monthlyVisitors && avgOrderValue && conversionRate) {
      ...
      const currentMonthlyRevenue = (visitors * (currentRate / 100) * orderValue);
      const currentAnnualRevenue = currentMonthlyRevenue * 12;

      // Projected improvement (assuming 14% increase in conversion rate)
      const improvementRate = 14; // 14% improvement
      const newConversionRate = currentRate * (1 + improvementRate / 100);
      const projectedMonthlyRevenue = (visitors * (newConversionRate / 100) * orderValue);
      const projectedAnnualRevenue = projectedMonthlyRevenue * 12;

      // Calculate increases
      const revenueIncrease = projectedAnnualRevenue - currentAnnualRevenue;
      const conversionIncrease = (visitors * 12 * ((newConversionRate - currentRate) / 100));

      setResults({
        currentRevenue: currentAnnualRevenue,
        projectedRevenue: projectedAnnualRevenue,
        revenueIncrease: revenueIncrease,
        conversionIncrease: improvementRate,
        conversionsPerYear: conversionIncrease,
