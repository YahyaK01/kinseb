...
export default function LandingPage() {
  ...
  // Default content (shows immediately) - Updated with unique content
  ...
  // Fetch content from specific Contentful entry to avoid conflicts
  useEffect(() => {
    const fetchContent = async () => {
      try {
        console.log('Fetching Landing Page content from specific Contentful entry...');
        
        // You can replace 'LANDING_PAGE_ENTRY_ID' with a specific entry ID
        // For now, it will search but with more specific criteria
        const allEntries = await client.getEntries();
        console.log('All available entries:', allEntries);
        
        // Look for landing page entry with very specific field combination
        const landingEntry = allEntries.items.find(item => 
          item.fields.shortTitle === "Transform Your Goals" ||
          (item.fields.point1 && item.fields.point2 && item.fields.point3 && 
           item.fields.contactInfoHeading && item.fields.formHeading)
        );
        
        console.log('Landing entry found:', landingEntry);
        
        if (landingEntry) {
          const landingData = landingEntry.fields;
          console.log('Landing data fields:', landingData);
          
          setContent({
            shortTitle: landingData.shortTitle || content.shortTitle,
            title: landingData.title || content.title,
            description: landingData.description || content.description,
            point1: landingData.point1 || content.point1,
            point2: landingData.point2 || content.point2,
            point3: landingData.point3 || content.point3,
            contactInfoHeading: landingData.contactInfoHeading || content.contactInfoHeading,
            email: landingData.email || content.email,
            phone: landingData.phone || content.phone,
            formHeading: landingData.formHeading || content.formHeading
          });
          
          console.log('Landing page content updated successfully!');
        } else {
          console.log('No specific landing page entry found, using defaults');
        }
      } catch (error) {
        console.error('Error fetching landing page content from Contentful:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const templateParams = {
        ...
        company_name: formData.companyName || 'Website Visitor',
        contact_number: formData.contactNumber || 'Not provided',
        message: formData.message,
        to_email: 'info@kinsebmarketing.com',
      };
