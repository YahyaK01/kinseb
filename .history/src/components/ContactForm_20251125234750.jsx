...
export default function LandingPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        console.log('Fetching form content from Contentful...');
        
        const allEntries = await client.getEntries();
        console.log('All available entries:', allEntries);
        
        const formEntry = allEntries.items.find(item => 
          item.fields.formHeading === "Grow Now!" ||
          (item.fields.fullNameLabel && item.fields.emailLabel && item.fields.buttonText)
        );
        
        console.log('Form entry found:', formEntry);
        
        if (formEntry) {
          const formData = formEntry.fields;
          console.log('Form data fields:', formData);
          
          setContent({
            formHeading: formData.formHeading || content.formHeading,
            fullNameLabel: formData.fullNameLabel || content.fullNameLabel,
            emailLabel: formData.emailLabel || content.emailLabel,
            companyLabel: formData.companyLabel || content.companyLabel,
            contactLabel: formData.contactLabel || content.contactLabel,
            messageLabel: formData.messageLabel || content.messageLabel,
            privacyText: formData.privacyText || content.privacyText,
            buttonText: formData.buttonText || content.buttonText
          });
          
          console.log('Form content updated successfully!');
        } else {
          console.log('No specific form entry found, using defaults');
        }
      } catch (error) {
        console.error('Error fetching form content from Contentful:', error);
      }
    };
